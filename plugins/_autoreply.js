let fs = require('fs') 
 ...
             } 
     })         
             await this.send2ButtonLoc(m.chat, 'https://telegra.ph/file/96a78933baa32ab4829ca.jpg', m.msg.contextInfo.expiration == 604800 ? '\n\nketik *.ephe* untuk matiin pesan sementaranya, biar tombolnya bisa dipake' : `Uhm.. iyaa ada apa @${m.sender.split`@`[0]}?`, wm, isBanned ? 'Unban' : banned ? 'Pemilik Bot' : 'Menu', 
 ...
         await this.sendButtonDoc(m.chat, !(m.isGroup || m.isPrems) && group ? 'hanya grup' : isBanned ? 'chat banned' : banned ? 'user banned' : 'Iyaa ada apa?', wm, !(m.isGroup || m.isPrems) && group ? 'donasi' : isBanned ? 'unban' : banned ? 'Minta owner, untuk membukanya' : 'donasi', !(m.isGroup || m.isPrems) && group ? '.donasi' : isBanned ? '.unban' : banned ? '.owner' : '.donasi',  m, { 
     quoted: m, 
 ...
          let uptime = clockString(_uptime)  
   const ultah = new Date('Juli 25, 2023 23:59:59')
 ...
        await this.setBio(`Ulang Tahun Mursid S : ${ohari} Hari ${ojam} Jam ${onet} Menit ${detek} Detik
Aktif selama : ${uptime} | Mode: ${global.opts['self'] ? 'Private' : setting.groupOnly ? 'Only Group' : 'Public'} | ву мυʀѕι∂-χмℓ`).catch(_ => _)  
